...
// Get current page or set to 1 if not provided
...
<!-- category tabs -->
?php include_once('includes/categorybadge.php'); ?
<!-- category tabs -->
    ...
    <div class="w-full">
        <form>
            <input type="search" name="search" id="search" class="w-full px-4 py-1 text-white bg-[#212121] rounded-full focus:outline-none placeholder-[#808080]"
                placeholder="search pleace or Near by ..." value="<?= htmlspecialchars($search_term) ?>">
        </div>
        <div>
...
<div class="2xl:container 2xl:mx-auto flex flex-wrap items-start justify-center pt-6 px-4 gap-6 mt-[30px]">
  ...
  <?php
    // Adjust the query based on whether a search term is provided
    if ($search_term) {
        // Include posts that match the search term
        $postquary = "SELECT * FROM post WHERE (title LIKE '%$search_term%' OR category LIKE '%$search_term%') ORDER BY id DESC LIMIT $result, $posts_per_page";
    } else {
        // Exclude posts with the "Hotels" and "Hospitals" categories for general listing
        $postquary = "SELECT * FROM post WHERE category NOT LIKE '%Hotels%' AND category NOT LIKE '%Resturents%'AND category NOT LIKE '%Wine Stores%' AND category NOT LIKE '%Fuel Station%' AND category NOT LIKE '%Hospitals%' ORDER BY id DESC LIMIT $result, $posts_per_page";
    }
...
<div class="flex justify-center items-center px-[4%] md:px-[10%] mt-[50px]">
  ...
  <?php
    // Adjust the total post count query based on search term
    if ($search_term) {
        $count_query = "SELECT * FROM post WHERE title LIKE '%$search_term%' OR category LIKE '%$search_term%'";
    } else {
        $count_query = "SELECT * FROM post WHERE category NOT LIKE '%Hotels%' AND category NOT LIKE '%Hospitals%'";
    }
    $count_result = mysqli_query($db, $count_query);
    $total_posts = mysqli_num_rows($count_result);
  ...
  ?>
    ...
    <ul class="flex gap-1">
      <?php
      $switch = ($page > 1) ? "" : "pointer-events-none";
      $nswitch = ($page < $total_pages) ? "" : "pointer-events-none";
      ?>
